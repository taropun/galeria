...
(function() {
    ...
    function fixLinks() {
        ...
        relatedImagesLink.href = galleryLink.href.replace(/\?p=\d+$/, '');
        ...
        reloadLink.addEventListener('click', function(e) {
            e.preventDefault();
            e.stopPropagation();
            var matches = /^\/s\/([0-9a-f]+)\/(\d+)-(\d+)$/.exec(window.location.pathname);
            var pageToken = matches[1];
            var galleryId = parseInt(matches[2], 10);
            var page = parseInt(matches[3], 10);
            var request = {'method': 'gtoken',
                           'pagelist': [[galleryId, pageToken, page]]};
            POST(apiUrl, JSON.stringify(request), function() {
                var response = JSON.parse(this.responseText);
                var galleryToken = response.tokenlist[0].token;
